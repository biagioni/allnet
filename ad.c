...
#define PROCESS_PACKET_LOCAL	1
#define PROCESS_PACKET_MGMT	2
#define PROCESS_PACKET_ALL	4

#define TRACE_REPLY_SIZE	sizeof (struct allnet_header_max) +        \
                                sizeof (struct allnet_mgmt_trace_reply) +  \
                                sizeof (struct allnet_header_mgmt)
                         ...
                         int * priority)
{
  ...
  if (msize < hs + sizeof (struct allnet_header_mgmt))
    return PROCESS_PACKET_DROP;
  struct allnet_header_mgmt * ahm = 
    (struct allnet_header_mgmt *) (message + hs);
  ...
  case ALLNET_MGMT_TRACE_PATH:
    *priority = EPSILON;
#if 0
    if (ahm->mgmt_type == ALLNET_MGMT_TRACE_REQ)
      *priority = EPSILON + 1;
    if (ahm->mgmt_type == ALLNET_MGMT_TRACE_PATH)
      *priority = EPSILON + 2;
#endif /* 0 */
                           ...
                           struct social_info * soc, int * priority)
{
  ...
  add_trace_info (packet, size, is_local);
...
#undef HEADER_SKIP
  ...
  if (ah->message_type == ALLNET_TYPE_MGMT)      /* AllNet management */
    return process_mgmt (packet, size, is_local, priority);
...
log_print ();
    ...
    case PROCESS_PACKET_LOCAL:   /* send only to alocal and acache */ 
      ...
      break;
    case PROCESS_PACKET_MGMT:    /* send only to aip */
      log_packet ("sending mgmt packet to aip", packet, psize);
      send_all (packet, psize, priority, write_pipes + 2, 1, "mgmt");
      break;
