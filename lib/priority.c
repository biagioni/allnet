...
{
  double v = ((double) value) / ((double) MAX_PRIORITY);
  if (str != NULL)
    printf ("%s: %f\n", str, v);
  else
    printf ("%f", v);
...
int divide (int dividend, int divisor)
{
  long long int product =
     ((long long int) MAX_PRIORITY) * ((long long int) dividend);
                      ...
                      int social_distance, int rate_fraction)
{
  ...
  if (is_local)
    return SEVEN_EIGHTS;  /* sender may have specified another priority */
  ...
  int hops_carried_priority = ONE_HALF;
  if (hops_already < 1)  /* should be local, but be sane anyway */
    hops_already = 1;
  if (hops_already <= 4)
    hops_carried_priority = MAX_PRIORITY - ONE_EIGHT * (hops_already - 1);
  ...
  int bits_priority = MAX_PRIORITY - power_half_fraction (dbits + 1);
  ...
  int rate_priority = MAX_PRIORITY - rate_fraction;
