...
/* hash n blocks, padding.
 * The result must be (at least) 64 bytes
 */
void sha512 (char * input, int bytes, char * result) 
{
  ...
  char * buffer = NULL;
  if (debugging)
    printf ("%d bytes, %d needed, full is %d = 128 * %d\n",
	    bytes, needed, full, blocks);
  buffer = malloc (full);
  if (buffer == NULL) {
    printf ("unable to allocate %d bytes for sha512 buffer\n", full);
    exit (1);
  }
  memcpy (buffer, input, bytes);
  memset (buffer + bytes, 0, full - bytes);
  buffer [bytes] = 0x80;  /* first bit must be one */
  write_int (buffer + (full - 8), bits);
#ifdef DEBUG_PRINT
  printf ("padded block: \n");
  print_data (buffer, full);
  printf ("sizeof hash is %ld, sizeof init_H is %ld\n",
          sizeof (uint512), sizeof (init_H));
#endif /* DEBUG_PRINT */
  memcpy (hash, init_H, sizeof (init_H));
  for (i = 0; i < blocks; i++) {
    compute_sha512 (((unsigned long long int *) (buffer + 128 * i)), hash, 0);
#ifdef DEBUG_PRINT
    printf ("hash is %016llx %016llx %016llx %016llx %016llx %016llx %016llx %016llx\n",
            hash.i [0], hash.i [1], hash.i [2], hash->i [3],
            hash->i [4], hash->i [5], hash->i [6], hash->i [7]);
